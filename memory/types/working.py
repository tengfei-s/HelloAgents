...
class WorkingMemory(BaseMemory):
    ...
    def retrieve(self, query: str, limit: int = 5,user_id: str = None, **kwargs) -> List[MemoryItem]:
        ...
        if user_id:
            filtered_memories = [m for m in active_memories if (m.metadata or {}).get("user_id") == user_id]
        ...
        for memory in filtered_memories:
            ...
            if query_lower in content_lower:
                ...
                if intersection:
                    ...
                    content_chars = set(content_lower.replace(" ", ""))
                    char_intersection = query_chars.intersection(content_chars)
                    if char_intersection:
                        keyword_score = len(char_intersection) / len(query_chars.union(content_chars)) * 0.6
    ...
    def _calculate_priority(self, memory_item):
    ...
    def _mark_deleted_in_heap(self, memory_id: str):
        pass
    def _update_heap_priority(self, memory):
        """更新堆中记忆的优先级"""
        # 简单实现：重建堆
        self.memory_heap = []
        for mem in self.memories:
